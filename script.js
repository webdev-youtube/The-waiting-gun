...
class Enemy {
    ...
    die() {
        ...
        const BASE_REWARDS = {
            NORMAL: 10, FAST: 8, TANK: 30, HYDRA: 20, HEALER: 24, SHIELD: 24
        };
        ...
        const moneyMult = (1 + gameState.upgrades.damageMultiplier * 0.02)
                        * Math.pow(1.12, gameState.upgrades.warChest); // WAR CHEST: +12% money per level (compounding)
    ...
    for (const bullet of bulletPool) {
        ...
        for (const enemy of enemyPool) {
            ...
            if (dist < size) {
                ...
                if (Math.random() * 100 < gameState.upgrades.explosiveRounds * 5) {
...
function getUpgradeCost(upgrade, currentLevel) {
    // FIELD RATIONS: each level reduces all upgrade costs by 3%, stacks multiplicatively
    const discount = Math.pow(0.97, gameState.upgrades.fieldRations || 0);

    const costs = {
        damageMultiplier: () => 8   * Math.pow(1.4,  currentLevel),
        fireLimitBreak:   () => 35  * Math.pow(1.6,  currentLevel),
        moreAmmo:         () => 20  * Math.pow(1.5,  currentLevel),
        captainAutofire:  () => 250,
        ...
        barbedWires:      () => 400,
        fieldPatch:       () => 90,
        emergencyRepairs: () => Math.floor(gameState.wallMaxHP * 1.2),
        warChest:         () => 80  * Math.pow(1.55, currentLevel),
        fieldRations:     () => 100 * Math.pow(1.65, currentLevel)
    };
    // Consumables and fieldRations itself are exempt from the discount
    const exempt = ['fieldPatch', 'emergencyRepairs', 'fieldRations'];
    const base = Math.floor(costs[upgrade]());
    return exempt.includes(upgrade) ? base : Math.floor(base * discount);
}
...
function buyUpgrade(upgradeName) {
    ...
    switch (upgradeName) {
        case 'fireLimitBreak': {
            ...
            if      (lv <= 5)  gameState.maxX += 1;
            else if (lv <= 10) gameState.maxX += 2;
            else if (lv <= 20) gameState.maxX += 4;
            else               gameState.maxX += 6;
            break;
        }
        case 'moreAmmo': {
            // Exponential gain — slow early, snowballs rapidly around level 10
            const lv  = gameState.upgrades[upgradeName];
            const gain = Math.max(1, Math.floor(Math.pow(1.2, lv)));
            gameState.bulletsPerSecond += gain;
            break;
        }
        case 'warChest':
        case 'fieldRations':
            // Passive — effect applied in die() and getUpgradeCost() respectively
            break;
